...
namespace Aoc2024.Day03
{
    public class Solution
    {
        ...
        private static int SolvePart1(string inputPath)
        {
            return Solve(
                inputPath,
                (program) => ExecuteCorrectInstructions(program, [instructionRegexes["mul"]])
            );
        }
        ...
        private static int Solve(string inputPath, Func<string, int> solver)
        {
            var program = string.Join(
                "",
                FileOpener
                    .ReadIntoSplitLines<string>($"day03/{inputPath}")
                    .Select((line) => line[0])
            );
            return solver(program);
        }

        private static int ExecuteCorrectInstructions(
            string program,
            string[] correctInstructionRegexes
        )
        {
        ...
        private static (string, int[])[] FindCorrectInstructions(
            string program,
            string[] correctInstructionRegexes
        )
        {
            ...
            return instructionMatches
                .Select(
                    (instructionMatch) =>
                    {
                        ...
                        if (operationName == "")
                        {
                            operationName = instructionMatch.Groups[3].Success
                                ? instructionMatch.Groups[3].Value
                                : instructionMatch.Groups[4].Value;
                        }
                        else
                        {
                            var operandsMatch = instructionMatch.Groups[2];
                            operands = operandsMatch
                                .Value.Split(",")
                                .Select((operandStr) => int.Parse(operandStr))
                                .ToArray();
                        }
        ...
        private static int SolvePart2(string inputPath)
        {
            return Solve(
                inputPath,
                (program) => ExecuteCorrectInstructions(program, [.. instructionRegexes.Values])
            );
        }
    }
}
